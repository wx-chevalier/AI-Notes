...
两个集合的距离，定义为两个集合间元素的最短距离，然后做集合C和集合D最短线段的垂直平分线，即得到了最优的超平面：
...
$$f(x)=max(f_1(x),\dots,f_n(x)) \\ f(x)=sup_{y \in A}g(x,y)$$
...
对于上面所说的凸优化的基本问题，可以带入Lagrange函数：

$$L(x,\lambda,v) = f_0(x) + \sum^m_{i=1}\lambda_if_i(x) + \sum_{j=1}^pv_jh_j(x)$$

对于固定的$$x$$，Lagrange函数即为关于$$\lambda$$和$$v$$的仿射函数。

然后对于该函数求下界：

$$g(\lambda,v)=inf_{x \in D}L(x,\lambda,v)=inf_{x \in D}( f_0(x) + \sum^m_{i=1}\lambda_if_i(x) + \sum_{j=1}^pv_jh_j(x))$$

如果没有下确界，定义：$$g(\lambda,v) = - \infty$$。根据定义，显然有对于$$\forall \lambda > 0, \forall v$$，若原优化问题有最优值$$p^*$$，则有$$g(\lambda,v) \le p^*$$，进一步可以得到Lagrange对偶函数为凸函数。
